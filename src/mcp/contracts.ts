...
export function cliEquivalentForTool(name: string, input: unknown): string {
  ...
  const map: Record<string, () => string> = {
    ...
    get_inbound_email: () => `emails inbox show ${id ?? "<inbound-id>"} --json`,
    clear_inbound_emails: () => "emails inbox clear --json",
    sync_inbox: () => "emails inbox sync --json",
    ...
    reply_to_email: () => `emails reply ${id ?? "<email-id>"} --json`,
    get_attachment: () => `emails inbox attachment ${id ?? "<attachment-id>"} --json`,
